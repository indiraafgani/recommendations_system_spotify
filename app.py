...
def recommend_cold_start(user_id, df_user, item_profiles, audio_features, df_all, N=10):
    ...
    if n_songs == 0: return get_popular_items(df_all, N=N, exclude=seen_ids)
    elif n_songs == 1:
        cbf_recs = recommend_cbf(user_id, df_user, item_profiles, audio_features, N=N//2)
        cbf_ids  = set(r[0] for r in cbf_recs)
        return cbf_recs + get_popular_items(df_all, N=N-N//2, exclude=seen_ids | cbf_ids)
    else:
...
section[data-testid="stSidebar"] .stButton > button { background:rgba(252,230,143,0.12) !important; border:1px solid rgba(252,230,143,0.28) !important; color:var(--yellow) !important; font-size:11.5px !important; border-radius:8px !important; }
section[data-testid="stSidebar"] .stButton > button:hover { background:rgba(252,230,143,0.24) !important; }
.stTextInput > div > div > input { background:var(--card) !important; border:2px solid var(--card-border) !important; color:var(--text-hi) !important; border-radius:14px !important; font-size:14px !important; padding:13px 18px !important; }
.stTextInput > div > div > input:focus { border-color:var(--purple) !important; }
.stTextInput > div > div > input::placeholder { color:var(--text-lo) !important; }
.stButton > button { background:linear-gradient(135deg,var(--purple) 0%,var(--purple-mid) 100%) !important; color:var(--yellow) !important; border:none !important; border-radius:14px !important; font-family:'Syne',sans-serif !important; font-weight:700 !important; font-size:13px !important; padding:13px 20px !important; }
...
/* Song card name/artist/match */
.song-title { font-family:'Syne',sans-serif; font-size:13px; font-weight:700; color:var(--text-hi); margin-bottom:4px; line-height:1.35; margin-top:8px; }
.song-artist { font-size:11.5px; color:var(--text-lo); margin-bottom:8px; }
.match-label { font-size:10px; font-weight:700; color:var(--purple); margin-bottom:4px; font-family:'Syne',sans-serif; letter-spacing:0.05em; text-transform:uppercase; }
.match-bar-bg { height:4px; background:var(--bg3); border-radius:2px; overflow:hidden; margin-bottom:4px; }
.match-bar-fill { height:100%; border-radius:2px; background:linear-gradient(90deg,var(--purple) 0%,var(--yellow-dim) 100%); }
/* card wrapper */
.song-card-wrapper { background:var(--card); border:1.5px solid var(--card-border); border-radius:18px; padding:14px; box-shadow:0 2px 10px var(--shadow); transition:all 0.22s ease; }
.song-card-wrapper:hover { border-color:var(--purple); }
/* expander list */
.rec-row { display:flex; align-items:center; gap:12px; padding:10px 0; border-bottom:1px solid var(--card-border); }
...
/* hide st.image caption/border */
.stImage { border-radius:12px !important; overflow:hidden !important; }
</style>
...
if user_id or find_btn:
    ...
    if not uid:
        ...
        # ── 3-column grid using st.columns + st.image ──
        cols = st.columns(3)
        for i, rec in enumerate(recs[:6]):
            ...
            with cols[i % 3]:
                ...
                if art_url:
                    ...
                    st.markdown(f'<div style="width:100%;aspect-ratio:1;background:{bg};border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:40px;">{emoji}</div>', unsafe_allow_html=True)
